...
**`contracts/adapters/phoenix` — still not implemented.** Still a bare `#[contract]` with a single `version() -> 1` function — no Phoenix DEX integration, no rebalancing/yield-comparison logic between multiple strategies. `strategy_router` is deliberately single-strategy today (see its own doc comments) — real multi-strategy allocation is future work, not implied here.
